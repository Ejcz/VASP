...
const biomeProportions = {      // Here you can modify how many of each biome generates first, randomly.
    desert: 1,
    forest: 1,
    plains: 1,
    jungle: 1,
...
let biomePosition = {           // All the positions where a biome is located
    desert: [],
    forest: [],
    plains: [],
    jungle: [],
...
for (let i = 0; i < nrHexes; i++) {     // It is first filled up with blank hexes for easier use later
    biomes.push('')
}

biomeTypes.forEach((biome) => {                             //Randomly generating biomes according to biomeProportions
...
function expand(b, isEnough) {
    biomePosition[b].forEach((position) => {                    // Marking adjacent hexes to each biome hex
        ...
        adjacent.forEach((pos) => {                             // Checks if one of the adjacent positions is the biome type that we want to expand, then filters it out.
            if (biomes[pos] !== b) {
        ...
        if (isEnough == true) {
            if (biomes[adjacent[a]] == '') {                    // Not overwriting
                biomes[adjacent[a]] = b
                biomePosition[b].push(adjacent[a])
            }
        } else {                                                // Overwriting
            if (adjacent[a] >= 0 && adjacent[a] < nrHexes) {    // Is the adjacent hex in the array or not
                if (biomePosition[biomes[adjacent[a]]].length > 9) {                            // Does the overwritten biome also have too little hexes
...
for (let i= 0; i < 12; i++) {           // Expanding each biome 11 times, with 10 it sometimes doesn't fill up the whole map
    expand('desert', true)
    expand('forest', true)
    expand('plains', true)
    expand('jungle', true)
...
for (let i = 0; i < 5; i++) {           // 4 times expanding the biomes with too little hexes. More times will produce a lot of lone hexes
    biomeTypes.forEach((biome) => {
        if (biomePosition[biome].length < 15) {     // Expanding each biome if it has less than 15 hexes. Will have to change if we increase the size of the map.
            expand(biome, false)
        }
    })
}
